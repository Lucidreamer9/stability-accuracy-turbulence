...
defaultopt = struct('Display','final','HessUpdate','bfgs','GoalsExactAchieve',1,'GradConstr',false,  ...
			...
			'rho',0.0100,'sigma',0.900,'tau1',3,'tau2', 0.1, 'tau3', 0.5,'StoreN',20,'OutputFile','');
...
fid = -1;
if(isfield(optim,'OutputFile') && ~isempty(optim.OutputFile))
    fid = fopen(optim.OutputFile, 'w');
    if(fid == -1)
        warning('fminlbfgs:file','Could not open output file: %s', optim.OutputFile);
    end
end

% Display column headers
if(strcmp(optim.Display,'iter'))
    header_str = '     Iteration  Func-count   Grad-count         f(x)         Step-size';
    disp(header_str);
    if(fid ~= -1)
        fprintf(fid, '%s\n', header_str);
    end
...
if(strcmp(optim.Display,'iter'))
        s=sprintf('     %5.0f       %5.0f       %5.0f       %13.6g    ',data.iteration,data.funcCount,data.gradCount,data.fInitial); 
        disp(s);
        if(fid ~= -1)
            fprintf(fid, '%s\n', s);
        end
...
while(true)
    ...
    data.fminimum = 0;% WQ I changed this 12/08/2023
    ...
    if(strcmp(optim.Display(1),'i')||strcmp(optim.Display(1),'p'))
        s=sprintf('     %5.0f       %5.0f       %5.0f       %13.6g   %13.6g',data.iteration,data.funcCount,data.gradCount,data.fInitial,data.alpha); 
        disp(s);
        if(fid ~= -1)
            fprintf(fid, '%s\n', s);
        end
